...
Feature: SQS Integration
  ...
  Background:
    Given AWS SNS client
      And AWS SQS client
	  And I purge queue "http://localhost:9324/queue1"

  Scenario: Publish
    Given I create a new topic "test1"
    And I subscribe endpoint "file://tmp?fileName=sns.log" with protocol "file" to topic "test1"
	And I subscribe endpoint "aws-sqs://queue1?queueUrl=http://localhost:9324/queue1&accessKey=&secretKey=&region=EU_WEST_1" with protocol "sqs" to topic "test1"
    ...
    Then I should see "Hello, World!" in queue "http://localhost:9324/queue1"

  Scenario: Raw Message Delivery
    Given I create a new topic "test2"
	And I subscribe endpoint "aws-sqs://queue1?queueUrl=http://localhost:9324/queue1&accessKey=&secretKey=&region=EU_WEST_1" with protocol "sqs" to topic "test2" as "subscription"
    ...
    Then The publish request should be successful
    Then I wait for 1 seconds
    And I get the message in queue "http://localhost:9324/queue1"
  ...
  Scenario: Message Attributes
	...
	And I subscribe endpoint "aws-sqs://queue1?queueUrl=http://localhost:9324/queue1&accessKey=&secretKey=&region=EU_WEST_1" with protocol "sqs" to topic "test3" as "subscription"
    And I set "RawMessageDelivery" for "subscription" to "true"
